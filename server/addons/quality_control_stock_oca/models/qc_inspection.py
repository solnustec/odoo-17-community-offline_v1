...
class QcInspection(models.Model):
    ...
    picking_id = fields.Many2one(
        comodel_name="stock.picking", compute="_compute_picking", store=True, string="Operación"
    )
    ...
    def object_selection_values(self):
        ...
        result.extend(
            [
                ("stock.picking", "Tipo de operación"),
                ("stock.move", "Movimiento de stock"),
                ("stock.lot", "Lote / Número de serie"),
            ]
        )
        return result
    ...
    def action_confirm(self):
        ...
        stock_quant = self.env['stock.quant'].sudo().with_context(active_test=False).search([
            ("product_id", "=", self.product_id.id),
            ("lot_id", "=", self.lot_id.id),
            ("location_id.replenish_location", "=", True),
        ]).write({"active": True})
