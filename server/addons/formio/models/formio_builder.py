...
class Builder(models.Model):
    ...
    def action_graph_forms(self):
        self.ensure_one()
        graph_view = self.env.ref('formio.view_formio_form_graph')
        return {
            'name': 'Form Analysis',
            'type': 'ir.actions.act_window',
            'res_model': 'formio.form.data',
            'view_mode': 'graph,tree,form',
            'views': [(graph_view.id, 'graph'), (False, 'tree'), (False, 'form')],
            'target': 'current',
            'domain': [('builder_id', '=', self.id)],
            'context': {'default_builder_id': self.id}
        }
