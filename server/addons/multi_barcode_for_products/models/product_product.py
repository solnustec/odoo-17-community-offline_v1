...
class ProductProduct(models.Model):
    ...
    def name_search(self, name, args=None, operator="ilike", limit=100):
        ...
        if name:
            domain = [
                ...
                ("multi_barcode_ids.product_multi_barcode", operator, name),
            ]
    ...
    #TODO revusar esto esta dando problemas al crear los productos
    # @api.model
    # def create(self, vals):
    #     """Creating multi barcodes"""
    #     res = super(ProductProduct, self).create(vals)
    #     res.multi_barcode_ids.update({"product_template_id": res.product_tmpl_id.id})
    #     return res

    def write(self, vals):
        """Actualizar códigos de barras múltiples evitando error de singleton"""
        res = super(ProductProduct, self).write(vals)
        for product in self:
            product.multi_barcode_ids.update(
