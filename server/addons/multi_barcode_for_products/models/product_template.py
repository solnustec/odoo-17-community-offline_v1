...
class ProductTemplate(models.Model):
    ...
    def write(self, vals):
        """Updating the multi barcodes"""
        res = super(ProductTemplate, self).write(vals)
        #siempre usar un for para los casos donde se actualizan varios registros
        for record in self:
            if record.template_multi_barcode_ids:
                record.template_multi_barcode_ids.update({
    ...
    def create(self, vals):
        """Creating the multi barcodes"""
        res = super(ProductTemplate, self).create(vals)
        if res.template_multi_barcode_ids and res.product_variant_id:
            res.template_multi_barcode_ids.update({
                'product_id': res.product_variant_id.id
            })
        return res
