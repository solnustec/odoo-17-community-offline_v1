...
class PaymentTransaction(models.Model):
    ...
    def action_demo_set_done(self):
        ...
        self._handle_notification_data('paymentez', notification_data)
    ...
    def _send_payment_request(self):
        ...
        if not self.token_id:
            raise UserError("Paymentez: " + _("The transaction is not linked to a token."))
    ...
    def _send_refund_request(self, **kwargs):
        ...
        refund_tx._handle_notification_data('paymentez', notification_data)
    ...
    def _send_capture_request(self, amount_to_capture=None):
        ...
        tx._handle_notification_data('paymentez', notification_data)
    ...
    def _get_tx_from_notification_data(self, provider_code, notification_data):
        ...
        if provider_code != 'paymentez' or len(tx) == 1:
            return tx

        reference = notification_data.get('reference')
        tx = self.search([('reference', '=', reference), ('provider_code', '=', 'paymentez')])
        if not tx:
    ...
    def _process_notification_data(self, notification_data):
        ...
        self.provider_reference = f'paymentez-{self.reference}'
        ...
        if state == 'pending':
            ...
            if self.capture_manually and not notification_data.get('manual_capture'):
                ...
                self._set_done()
                self.sudo().write({'card_info': notification_data.get('data', False)})
