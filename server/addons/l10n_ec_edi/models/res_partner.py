...
class ResPartner(models.Model):
    ...
    def _commercial_fields(self):
        return super(ResPartner, self)._commercial_fields() + ['l10n_ec_taxpayer_type_id',
                                                               'l10n_ec_related_party']
    ...
    #     if not ctx.get('show_all_partner_types'):
    #         args = args + [('type', '=', 'contact')]
    #
    #     # Construir dominio para buscar por nombre o vat cuando se ingrese texto
    #     domain = list(args)
    #     if name:
    #         domain += ['|', ('name', operator, name), ('vat', operator, name)]
    #
    #     partners = self.search(domain, limit=limit)
    #     # Devolver tuplas con la etiqueta que incluye el VAT
    #     result = []
    #     for partner in partners:
    #         display = f"{partner.name} ({(partner.vat or '').strip()})" if partner.vat else partner.name
    ...
    def name_search(self, name='', args=None, operator='ilike', limit=100):
        args = args or []
        ctx = dict(self.env.context or {})
        # Por defecto limitar a partners de tipo 'contact'.
        # Si se necesita incluir direcciones (invoice/delivery) desde alguna vista,
        # pasar context: {'show_all_partner_types': True}
        if not ctx.get('show_all_partner_types'):
            args = args + [('type', 'in', ['contact'])]

        res = super(ResPartner, self).name_search(name=name, args=args, operator=operator,
                                                   limit=limit)
        for i in range(len(res)):
            partner = self.browse(res[i][0])
            if partner.vat:
                display_name = f"{partner.name} ({partner.vat.strip()})"
                res[i] = (res[i][0], display_name)
        return res
