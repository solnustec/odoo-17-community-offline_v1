...
class Applicant(models.Model):
    ...
    identification = fields.Char("Identificación", size=10, required=False, index='trigram', help="Introduce tu cédula")
    ...
    def _compute_application_count(self):
        ...
        if not any(self._ids):
            ...
            return
        self.flush_recordset(['email_normalized', 'partner_phone_sanitized', 'partner_mobile_sanitized', 'identification'])
        self.env.cr.execute("""
            SELECT
                id,
                (
                    ...
                    WHERE a.id != sub.id
                     AND ((a.email_normalized <> '' AND sub.email_normalized = a.email_normalized)
                       ...
                       OR (a.identification <> '' AND sub.identification = a.identification)
    ...
    def _get_similar_applicants_domain(self):
        ...
        if self.identification:
            domain = expression.OR([domain, [('identification', '=', self.identification)]])
