...
function getWKHTMLTOPDF_MESSAGES(status) {
    ...
    const _status = {
        ...
        workers: _t(
            "You need to start Odoo with at least two workers to print a pdf version of the report."
        ),
    };
...
function getReportUrl(action, type) {
    ...
    if (action.data && JSON.stringify(action.data) !== "{}") {
        // build a query string with `action.data` (it's the place where report
