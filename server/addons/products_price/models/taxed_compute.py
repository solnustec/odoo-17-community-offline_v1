...
class AccountTaxPython(models.Model):
    ...
    # def compute_all(self, price_unit, currency=None, quantity=1.0, product=None, partner=None, is_refund=False, handle_price_include=True, include_caba_tags=False, fixed_multiplicator=1, reward=False):
    #
    #     # if product and product.ecommerce_discount and not reward:
    #     #     model = self.env['product.template'].sudo()
    #     #     price_unit = model.calculate_adjusted_price(product, price_unit, quantity)
    #
