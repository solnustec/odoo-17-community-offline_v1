...
class AccountAsset(models.Model):
    ...
    move_line_check = fields.Boolean(
        compute="_compute_move_line_check", string="Tiene asientos contables"
    )
    name = fields.Char(
    ...
    purchase_value = fields.Monetary(
        required=True,
        help="Este monto representa el valor inicial del activo."
        "\nLa base de depreciación se calcula así:"
        "\nValor de compra - Valor residual.",
    )
    salvage_value = fields.Monetary(
        help="Valor estimado que un activo tendrá al venderlo "
        "al final de su vida útil.\n"
        "Se usa para determinar los importes de depreciación.",
    )
    depreciation_base = fields.Monetary(
        compute="_compute_depreciation_base",
        store=True,
        help="Este monto representa la base de depreciación "
        "del activo (Valor de compra - Valor residual).",
    )
    ...
    value_depreciated = fields.Monetary(
        compute="_compute_depreciation",
        string="Valor Depreciado Acumulado",
        store=True,
    )
    note = fields.Text()
    ...
    date_start = fields.Date(
        string="Fecha de inicio del activo",
        required=True,
        help="Debe añadir manualmente las líneas de depreciación "
        "de ejercicios anteriores si la fecha de inicio de depreciación "
        "es distinta a la fecha para la cual se deben generar los asientos. "
    )
    date_remove = fields.Date(string="Fecha de baja del activo", readonly=True)
    state = fields.Selection(
        ...
        default="draft",
        copy=False,
        help="Cuando se crea un activo, su estado es 'Borrador'.\n"
        "Si se confirma, pasa a 'En curso' y las líneas de depreciación "
        "pueden contabilizarse.\n"
        "Si se contabiliza la última línea, pasa a 'Cerrado'.\n"
        "Cuando se generan los asientos de baja, pasa a 'Retirado'. "
    )
    ...
    method = fields.Selection(
        ...
        readonly=False,
        store=True,
        help="Seleccione el método para calcular las líneas de depreciación.\n"
             "  * Lineal: Base de depreciación / Número de depreciaciones.\n"
             "  * Lineal-límite: Lineal hasta el valor residual.\n"
             "  * Decreciente: Valor residual * Factor decreciente.\n"
             "  * Decreciente-Lineal (solo si el tiempo es en años): pasa a lineal "
             "cuando la depreciación lineal anual supera a la decreciente anual.\n"
             "  * Decreciente-límite: Decreciente hasta el valor residual.",
    )
    method_number = fields.Integer(
        ...
        readonly=False,
        store=True,
        help="Número de años necesarios para depreciar el activo",
    )
    method_period = fields.Selection(
        selection=lambda self: self.env["account.asset.profile"]._selection_method_period(),
    ...
    method_progress_factor = fields.Float(
        string="Factor decreciente",
        compute="_compute_method_progress_factor",
        readonly=False,
        store=True,
    )
    method_time = fields.Selection(
        selection=lambda self: self.env["account.asset.profile"]._selection_method_time(),
        string="Método de tiempo",
        compute="_compute_method_time",
        readonly=False,
        store=True,
        help="Seleccione cómo calcular las fechas y el número de líneas.\n"
             "  * Número de años: Especifique los años.\n"
    ...
    use_leap_years = fields.Boolean(
        ...
        "calculará como base de amortización / (fecha de finalización de la amortización - "
    ...
    prorata = fields.Boolean(
        ...
        readonly=False,
        store=True,
        help="Indica que la primera entrada de depreciación para este activo "
        "debe realizarse a partir de la fecha de inicio de la depreciación en lugar de "
    ...
    carry_forward_missed_depreciations = fields.Boolean(
        string="Acumular depreciaciones omitidas",
        help="""Si se crea un activo en un período fiscal que ya está cerrado
        el importe acumulado de las amortizaciones que no se pueden contabilizar se
        trasladará a la primera línea de amortización del período abierto actual.""",
    )
    ...
    def _check_method(self):
        if self.filtered(
            ...
            raise UserError(
                _("El método Decreciente-Lineal solo se admite para tiempo en años.")
            )
    ...
    def _check_profile_change(self):
        if self.depreciation_line_ids.filtered("move_id"):
            raise UserError(
                _(
                    "No puede cambiar el perfil de un activo con asientos contables "
                )
            )
    ...
    def unlink(self):
        for asset in self:
            if asset.state != "draft":
                raise UserError(_("Solo puede eliminar activos en estado borrador."))
            if asset.depreciation_line_ids.filtered(
                lambda r: r.type == "depreciate" and r.move_check
            ):
                raise UserError(
                    _(
                        "No puede eliminar un activo que tenga depreciaciones contabilizadas."
                    )
                )
