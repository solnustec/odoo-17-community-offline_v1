...
class AccountMove(models.Model):
    ...
    def unlink(self):
        ...
        if deprs and not self.env.context.get("unlink_from_asset"):
            raise UserError(
                _(
                    "No está permitido eliminar un asiento contable "
                    "vinculado a un activo."
                    "\nDebe eliminar dichos asientos desde el activo."
                )
            )
    ...
    def write(self, vals):
        if set(vals).intersection(FIELDS_AFFECTS_ASSET_MOVE):
            ...
            if deprs:
                raise UserError(
                    _(
                        "No puede modificar un asiento contable "
                        "vinculado a una línea de depreciación de un activo."
                    )
                )
    ...
    def action_post(self):
        ...
        for move in self:
            # Fix: only supplier invoices generate assets
            ...
            for aml in move.line_ids.filtered(
                ...
                if not aml.name:
                    raise UserError(
                        _("El nombre del activo debe definirse en la etiqueta de la línea.")
                    )
                if aml.asset_id:
                    continue
                ...
                if aml.product_id:
                    asset.write({"product_id": aml.product_id.product_tmpl_id.id})

            created_assets = move.line_ids.filtered("asset_profile_id").mapped("asset_id")
            if created_assets:
                asset = created_assets[:1]
                message = _(
                    "Esta factura creó el(los) activo(s): %s",
                    Markup(
                        """<a href=# data-oe-model=account.asset data-oe-id={}"""
                        """>{}</a>""".format(asset.id, asset.display_name)
                    ),
                )
...
class AccountMoveLine(models.Model):
    ...
    @api.depends("account_id", "asset_id", "asset_profile_id")
    def _compute_asset_profile(self):
        for rec in self:
            if rec.asset_profile_id:
                continue
        ...
        for vals in vals_list:
            ...
            if not move.is_sale_document():
                if vals.get("asset_id") and not self.env.context.get("allow_asset"):
                    raise UserError(
                        _(
                            "No está permitido vincular "
                            "un asiento contable a un activo."
                            "\nDebe generar dichos asientos desde el activo."
                        )
                    )
        ...
        if set(vals).intersection(FIELDS_AFFECTS_ASSET_MOVE_LINE) and not (
            ...
            for move_line in self.filtered(lambda r: not r.move_id.is_sale_document()):
                ...
                if linked_asset:
                    raise UserError(
                        _(
                            "No puede modificar un apunte contable "
                            "vinculado a una línea de depreciación de un activo."
                        )
                    )
