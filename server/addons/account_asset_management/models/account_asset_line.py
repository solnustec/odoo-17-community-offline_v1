...
class AccountAssetLine(models.Model):
    ...
    previous_id = fields.Many2one(
        ...
        string="Línea de depreciación anterior",
        readonly=True,
    )
    ...
    depreciation_base = fields.Monetary(
        ...
        string="Base de depreciación",
    )
    amount = fields.Monetary(required=True)
    remaining_value = fields.Monetary(
        compute="_compute_values",
        string="Depreciación del siguiente período",
        store=True,
    )
    depreciated_value = fields.Monetary(
        compute="_compute_values",
        string="Monto ya depreciado",
        store=True,
    )
    line_date = fields.Date(string="Fecha", required=True)
    line_days = fields.Integer(string="Días", readonly=True)
    move_id = fields.Many2one(
        comodel_name="account.move",
        string="Asiento de depreciación",
        readonly=True,
        check_company=True,
    )
    move_check = fields.Boolean(
        compute="_compute_move_check", string="Publicado", store=True
    )
    type = fields.Selection(
        selection=[
            ("create", "Base de depreciación"),
            ("depreciate", "Depreciación"),
            ("remove", "Baja de activo"),
        ],
        readonly=True,
        default="depreciate",
    )
    init_entry = fields.Boolean(
        string="Asiento de saldo inicial",
        help="Marque esta casilla para las entradas de ejercicios  "
        "fiscales anteriores para las que Odoo no ha generado asientos contables.",
    )
