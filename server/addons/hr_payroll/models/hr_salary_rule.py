...
class HrSalaryRule(models.Model):
    ...
    sequence = fields.Integer(required=True, index=True, default=5,
        help='Order to render in report')
    sequence_order = fields.Integer(required=True, index=True, default=5,
...
result = contract.wage * 0.10''')
    ...
    can_be_imported = fields.Boolean(string='¿Puede ser importado?', default=False)
...
- Error: %s""",
    ...
    def _compute_rule(self, localdict):
        ...
        if self.amount_select == 'fix':
            try:
                return localdict.get('result_percent_depends', 0.0), self.amount_fix or 0.0, float(safe_eval(self.quantity, localdict)), 100.0
            except Exception as e:
                self._raise_error(localdict, _("Wrong quantity defined for:"), e)
        if self.amount_select == 'percentage':
            try:
                return (localdict.get('result_percent_depends', 0.0), float(safe_eval(self.amount_percentage_base, localdict)),
            ...
            try:
                ...
                return localdict.get('result_percent_depends', 0.0), localdict['result'], localdict.get('result_qty', 1.0), localdict.get('result_rate', 100.0)
