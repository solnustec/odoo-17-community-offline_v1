...
class HrPayslip(models.Model):
    ...
    lines_two = fields.One2many(
        'hr.payslip.import.save.lines', 'payslip_id', string='Payslip Lines 2',
        store=True,
        readonly=False
    )
    ...
    @api.constrains('struct_id', 'employee_id', 'date_from', 'date_to')
    def _check_duplicate_payslip(self):

        for payslip in self:
            if payslip.struct_id and payslip.employee_id and payslip.date_from and payslip.date_to:
                domain = [
                ...
                if existing_payslip:
                    raise ValidationError(_(
                        'Ya existe una nómina para el empleado %s '
                        'con la estructura %s en el período del %s al %s.\n'
                        'Nómina existente: %s'
                    ) % (
                                              ...
                                              existing_payslip.name or existing_payslip.number
                                          ))

    @api.depends('contract_id', 'struct_id')
    def _compute_date_from(self):
        for payslip in self:
            if payslip.struct_id and payslip.struct_id.name == "Liquidaciones":
                if payslip.employee_id and payslip.employee_id.contract_id and payslip.employee_id.contract_id.date_end:
                    payslip.date_to = payslip.employee_id.contract_id.date_end
                    if payslip.date_to:
                        first_day_of_month = payslip.date_to.replace(day=1)
                        payslip.date_from = first_day_of_month
                if payslip.employee_id and payslip.employee_id.contract_id and not payslip.employee_id.contract_id.date_end:
                    raise UserError('EL empleado no es liquidable, primero cierre su contrato!')
    ...
    def _compute_date_to(self):
        for payslip in self:
            # 1. Obtener date_to desde el contexto si está definido
            default_date_to = self.env.context.get('default_date_to')
            if default_date_to:
                payslip.date_to = default_date_to
            else:
                if payslip.date_from and isinstance(payslip.date_from, date):
                    delta = payslip._get_schedule_timedelta()
                    payslip.date_to = payslip.date_from + delta
                else:
                    payslip.date_to = False  # o puedes dejarlo en None

            # 2. Validar todo antes de comparar fechas
            if (
                    payslip.contract_id and
                    isinstance(payslip.date_from, date) and
                    isinstance(payslip.date_to, date) and
                    isinstance(payslip.contract_id.date_start, date) and
                    payslip.contract_id.date_end and isinstance(payslip.contract_id.date_end, date) and
                    payslip.date_from >= payslip.contract_id.date_start and
                    payslip.date_from < payslip.contract_id.date_end and
    ...
    def get_months_in_spanish(self, date_from, date_to):
        """Devuelve los meses en español entre dos fechas."""
        ...
        while current_date <= date_to:
            month_name = current_date.strftime('%B')  # Obtener mes en inglés
            month_name_es = self.MONTHS_ES.get(month_name, month_name)  # Traducir a español
            months.append(month_name_es)
    ...
    def compute_sheet(self, popup_verify = False):
    ...
    def _get_localdict(self):
        ...
        for line in self.input_line_ids:
            if line.code:
                if line.code not in input_dict:
                    input_dict[line.code] = []  # Inicializar como lista si no existe
                input_dict[line.code].append(line)
        ...
        localdict = {
            ...
            **{
                ...
                'lines': {line.name: line for line in self.lines_two if line.name},
                'is_import': self.env['ir.config_parameter'].sudo().get_param(
                    'hr_payroll.enable_import')
            }
        }
        return localdict

    def _get_rule_name(self, localdict, rule, employee_lang, date=None):
        extra = ""
        if localdict.get('result_name'):
            if date:
                localdict['result_name'] = localdict['result_name'] + " (%s)" % (date)
            return localdict['result_name']

        predefined_rule_names = {
        ...
        if rule.code == 'BASIC':
            if rule.name == "Double Holiday Pay":
                return _("Double Holiday Pay")
            if rule.struct_id.name == "CP200: Employees 13th Month":
                return _("Prorated end-of-year bonus")
            return predefined_rule_names.get(rule.code, rule.name)

        if rule.code in predefined_rule_names:
            return predefined_rule_names[rule.code]

        if date:
            extra = " %s" % date or ""
        return rule.with_context(lang=employee_lang).name + extra
    ...
    def _get_payslip_lines(self):
        ...
        employee_ids = self.mapped('employee_id.identification_id')

        valid_employee_ids = [emp_id for emp_id in employee_ids if emp_id]

        import_save_lines = {}
        if valid_employee_ids:
            import_lines = self.env['hr.payslip.import.save.lines'].sudo().search([
                ('payslip_id', 'in', payslip_ids),
                ('employee_id', 'in', valid_employee_ids)
            ])

            # Crear diccionario indexado para acceso O(1)
            for line in import_lines:
                key = (line.payslip_id.id, line.name, line.employee_id)
                import_save_lines[key] = line.value

        struct_ids = self.mapped('struct_id.id')

        # Obtener las estructuras y sus reglas
        payroll_structures = self.env['hr.payroll.structure'].browse(struct_ids)

        rules_by_struct = {}
        for struct in payroll_structures:
            rules_by_struct[struct.id] = sorted(struct.rule_ids, key=lambda x: x.sequence_order)
        ...
        for payslip in self:
            if not payslip.contract_id:
                raise UserError(_(
                    "There's no contract set on payslip %s for %s. Check that there is at least a contract set on the employee form.",
            ...
            employee_identification = localdict['employee'].identification_id

            # Usar reglas pre-cargadas en lugar de acceder a payslip.struct_id.rule_ids
            rules = rules_by_struct.get(payslip.struct_id.id, [])

            for rule in rules:
                if rule.name in blacklisted_rule_ids:
                    continue

                # Resetear variables locales
                localdict.update({
                    'result_percent_depends': None,
                    'result': None,
                    'result_qty': 1.0,
                    'result_rate': 100,
                    'result_name': False
                ...
                # PROCESAR REGLA
                percent_depends, amount, qty, rate = rule._compute_rule(localdict)

                if (rule.can_be_imported and employee_identification) or payslip.is_history_payslip:
                    import_key = (payslip.id, rule.name, employee_identification)
                    imported_value = import_save_lines.get(import_key)
                    if imported_value:
                        amount = imported_value

                # PROCESAR DIFERENTES TIPOS DE AMOUNT CON BLACKLIST GRANULAR
                processed_lines = self._process_rule_amount(
                    rule, amount, percent_depends, qty, rate,
                    localdict, payslip, employee_lang, rules_dict, result_rules_dict,
                    blacklisted_rule_ids, employee_identification, import_save_lines
                )

                line_vals.extend(processed_lines)

        return line_vals

    def _process_rule_amount(self, rule, amount, percent_depends, qty, rate,
                             localdict, payslip, employee_lang, rules_dict, result_rules_dict,
                             blacklisted_items=None, employee_identification=None, import_save_lines=None):

        if blacklisted_items is None:
            blacklisted_items = []
        ...
        if isinstance(amount, (list, models.Model)) and amount:
            ...
            for item in amount:
                item_id = getattr(item, 'name', None)

                if item_id and item_id in blacklisted_items:
                    continue

                # También verificar por otros identificadores si no tiene ID
                if not item_id:
                    item_identifier = getattr(item, 'code', None) or getattr(item, 'name', None)
                    if item_identifier and item_identifier in blacklisted_items:
                        continue

                filtered_amount.append(item)

            # Si todos los items están blacklisted, no procesar nada
            if not filtered_amount:
                return []

            # Procesar solo los items no blacklisted
            for item in filtered_amount:

                item_amout = round(item.amount, 2)

                # Calcular amount con percent_depends
                if percent_depends is None:
                    amount_val = float(item_amout)
                else:
                    amount_val = float(item_amout) * percent_depends / 100

                # Actualizar localdict solo si no es del mismo tipo
                if rule.name not in localdict.get('same_type_input_lines', {}):
                    previous_amount = localdict.get(rule.code, 0.0)


                    tot_rule = item_amout * qty * rate / 100.0
                    localdict[rule.name] = tot_rule

                    result_rules_dict[rule.name] = {
                        'total': tot_rule, 'amount': item_amout,
                    ...
                    rule_name = payslip._get_rule_name(localdict, rule, employee_lang, item.date)
                ...
                if rule_name in blacklisted_items:
                    continue

                if (rule.can_be_imported and employee_identification) or payslip.is_history_payslip:
                    import_key = (payslip.id, rule_name, employee_identification)
                    imported_value = import_save_lines.get(import_key)
                    if imported_value:
                        amount_val = imported_value
            ...
            # Batch update de items
            if items_to_update:
                for item in items_to_update:
                    if isinstance(item, models.BaseModel):
                        item.sudo().write({'payslip_id': payslip.id})

        # PROCESAR VALORES NUMÉRICOS
        elif isinstance(amount, (int, float)) or (isinstance(amount, str) and amount.replace('.', '', 1).isdigit()):
            if percent_depends is None:
                amount = round(float(amount), 2)
            else:
                amount = round(float(amount), 2) * percent_depends / 100
            non_numeric_value = None

        # PROCESAR VALORES NO NUMÉRICOS
        else:
            non_numeric_value = amount
            amount = 0.0

        # CREAR LÍNEA ÚNICA SI NO SE PROCESÓ LISTA
        if not is_list_processed:
            tot_rule = round(float(amount), 2) * qty * rate / 100.0

            # Solo actualizar localdict si no es del mismo tipo
            if rule.code not in localdict.get('same_type_input_lines', {}):
                previous_amount = localdict.get(rule.name, 0.0)
                localdict[rule.code] = tot_rule
                result_rules_dict[rule.name] = {
                    'total': tot_rule, 'amount': round(float(amount), 2),
                    'quantity': qty, 'rate': rate
                }
                rules_dict[rule.name] = rule
                localdict = rule.category_id._sum_salary_rule_category(
