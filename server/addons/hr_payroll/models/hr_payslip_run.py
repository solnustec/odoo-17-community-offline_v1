...
class HrPayslipRun(models.Model):
    ...
    first_struct_id = fields.Many2one(
        'hr.payroll.structure',
        string='Primera Estructura',
        compute='_compute_first_struct_id',
        store=False
    )

    @api.depends('slip_ids.struct_id')
    def _compute_first_struct_id(self):
        for record in self:
            if record.slip_ids:
                record.first_struct_id = record.slip_ids[0].struct_id
            else:
                record.first_struct_id = False
    ...
    def action_download_payslip_xlsx(self):
        ...
        download_url = '/payroll/download_payslip_xlsx/{}'.format(self.id)
        return {
