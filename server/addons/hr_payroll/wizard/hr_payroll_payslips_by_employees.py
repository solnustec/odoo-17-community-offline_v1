...
class HrPayslipEmployees(models.TransientModel):
    ...
    def _get_available_contracts_domain(self, obj=False):
        if obj:
            return [
                ('contract_ids.state', 'in', ('open', 'close')),
                ('company_id', '=', self.env.company.id),

                # Contrato comenzó antes o durante el cálculo
                ('contract_ids.date_start', '<=', obj.date_end),

                # El contrato sigue vigente o terminó después de que empieza el cálculo
                '|',
                ('contract_ids.date_end', '=', False),
                ('contract_ids.date_end', '>=', obj.date_start),
            ]
    ...
    def _get_employees(self):
        ...
        if active_id:
            model_run = self.env['hr.payslip.run'].browse(active_id)
            employee_ids = self.env['hr.employee'].sudo().search(self._get_available_contracts_domain(model_run))
            return employee_ids
    ...
    def compute_sheet(self):
        ...
        if not active_id:
            from_date, end_date = self._get_dates(context, today)
            payslip_run = self.env['hr.payslip.run'].create({
                'name': self._generate_batch_name(from_date, end_date, today),
                'date_start': from_date,
                'date_end': end_date,
            })
        else:
        ...
        employees -= payslip_run.slip_ids.mapped('employee_id')  # Prevent duplicates
        if not employees:
            return self._get_success_action(payslip_run)

        self._process_work_entries(payslip_run, employees)
        self._generate_payslips(payslip_run, employees)
    ...
    def _get_dates(self, context, today):
        from_date = fields.Date.to_date(context.get('default_date_start'))
        end_date = fields.Date.to_date(context.get('default_date_end'))
        return from_date, end_date

    def _generate_batch_name(self, from_date, end_date, today):
        first_day = today + relativedelta(day=1)
        last_day = today + relativedelta(day=31)
        return (
            from_date.strftime('%B %Y')
            if from_date == first_day and end_date == last_day
            else _('From %s to %s', format_date(self.env, from_date), format_date(self.env, end_date))
        )

    def _get_success_action(self, payslip_run):
        return {
            'type': 'ir.actions.act_window',
            'res_model': 'hr.payslip.run',
            'views': [[False, 'form']],
    ...
    def _process_work_entries(self, payslip_run, employees):
        pass
        # contracts = employees._get_contracts(
        #     payslip_run.date_start, payslip_run.date_end, states=['open', 'close']
        # ).filtered('active')
        ...
        # contracts.generate_work_entries(payslip_run.date_start, payslip_run.date_end)
        # self._validate_work_entries(payslip_run, employees)

    def _validate_work_entries(self, payslip_run, employees):
        batch_size = 200
        ...
        for offset in range(0, total_employees, batch_size):
            batch_ids = employee_ids[offset:offset + batch_size]
            work_entries = work_entry_model.search([
                ('date_start', '<=', payslip_run.date_end + relativedelta(days=1)),
                ('date_stop', '>=', payslip_run.date_start + relativedelta(days=-1)),
                ('employee_id', 'in', batch_ids),
            ])
            invalid_entries = work_entries.filtered(lambda entry: entry.state != 'validated')
            if invalid_entries._check_if_error():
                raise UserError(_("There are unvalidated work entries."))

    def _generate_payslips(self, payslip_run, employees, structure_id=False, is_history_payslip=False):
        ...
        contracts, employees_not_found = self.get_contractsCustom(
            employees, payslip_run.date_start, payslip_run.date_end
        )
