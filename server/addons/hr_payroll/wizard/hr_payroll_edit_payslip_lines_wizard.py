...
class HrPayrollEditPayslipLinesWizard(models.TransientModel):
    ...
    def recompute_following_lines(self, line_id):
        ...
        for line in sorted(self.line_ids, key=lambda x: x.sequence_order):
            if remove_lines and line.code in self.payslip_id.line_ids.mapped('code'):
                lines_to_remove.append((2, line.id, 0))
            else:
                rules_dict[line.name] = line.salary_rule_id
                if line == wizard_line:
                    line._compute_total()
                    remove_lines = True
                blacklisted_rule_ids.append(line.name)
                localdict[line.name] = line.total
                result_rules_dict[line.name] = {'total': line.total, 'amount': line.amount, 'quantity': line.quantity, 'rate': line.rate}
    ...
    def recompute_worked_days_lines(self):
        ...
        lines = sorted(self.line_ids, key=lambda x: x.sequence_order)
        if not lines:
            return False
        lines[0].update({
            'amount': total_amount,
...
class HrPayrollEditPayslipLine(models.TransientModel):
    ...
    data_extra = fields.Char(string='Datos')
    ...
    is_edited = fields.Boolean(string='Esta editado', default=False)
